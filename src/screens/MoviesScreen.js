...
export default function MoviesScreen({ route, navigation }) {
  ...
  async function init() {
    try {
      ...
      let loaded = [];
      if (loginType === "xtream") {
        loaded = await loadXtreamContent(server, username, password, "vod");
      } else {
        const all = await loadM3UAll(m3uUrl);
        loaded = filterM3UBySection(all, "vod");
      }

      setAllItems(loaded);

      const favs = await getFavorites();
      setFavoriteIds(favs.filter((x) => x?.section === "vod").map((x) => `vod:${x?.id}`));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function applyFilter() {
    ...
    if (leftMode === "favorites") {
      const favs = await getFavorites();
      base = favs.filter((x) => x?.section === "vod");
    } else if (leftMode === "recent") {
      const hist = await getHistory();
      base = getRecentItemsBySection(hist, "vod");
          ...
          <View style={styles.rightPane}>
            {selectedMovie && (
              <View style={styles.summaryBox}>
                ...
                <TouchableOpacity style={styles.watchBtn} onPress={() => openMovie(selectedMovie)}>
