...
function buildSeriesCategories(series) {
  ...
  Object.keys(grouped).forEach((group, index) => {
    categories.push({ id: `series_group_${index}`, name: group.toUpperCase(), items: grouped[group] });
  });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.card} onPress={() => {
                ...
                <Text style={styles.group} numberOfLines={1}>{item.group || "Série"}</Text>
              </TouchableOpacity>
            )}
          />
        </View>
        ...
        <View style={styles.rightPanel}>
          {selectedSeries ? (
            <>
              {selectedSeries.logo ? <Image source={{ uri: selectedSeries.logo }} style={styles.detailPoster} /> : <View style={styles.detailPoster} />}
            ...
            <Text style={styles.empty}>Selecione uma série</Text>
          )}
        </View>
      </View>
      ...
      <Modal visible={showPlayer} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: "#06111d" }}>
          ...
          <View style={{ padding: 10 }}>
            <VideoPlayer url={selectedSeries?.url} title={selectedSeries?.name} brand="MUNDO PLAY TV" />
          </View>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
